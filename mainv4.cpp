...
int main(){   
    ...
    BatchLayer batch(8, inputHeight, inputWidth);
    ...
    BatchLayer batch1(16, inputHeight / 2, inputWidth / 2);
    ...
    BatchLayer batch2(32, inputHeight / 4, inputWidth / 4);
    ...
    for(int k = 0;k < maxEpoch;k++){
        ...
        for(int i = 0;i < maxIter;i++){
            ...
            batch.forward(conv.output);
            thiRel.forward(batch.output);
            pool.forward(thiRel.output);
            conv1.forward(pool.output);
            batch1.forward(conv1.output);
            thiRel1.forward(batch1.output);
            pool1.forward(thiRel1.output);
            conv2.forward(pool1.output);
            batch2.forward(conv2.output);
            thiRel2.forward(batch2.output);
            span.forward(thiRel2.output);
            ...
            thiRel2.backward(span.error);
            batch2.backward(thiRel2.error);
            conv2.backward(batch2.error);
            pool1.backward(conv2.error);
            thiRel1.backward(pool1.error);
            batch1.backward(thiRel1.error);
            conv1.backward(batch1.error);
            pool.backward(conv1.error);
            thiRel.backward(pool.error);
            batch.backward(thiRel.error);
            conv.backward(batch.error);
        ...
        for(int i = 0;i < maxIter;i++){
            ...
            inp.forward(tar);
            conv.forward(inp.output);
            batch.testForward(conv.output);
            thiRel.forward(batch.output);
            pool.forward(thiRel.output);
            conv1.forward(pool.output);
            batch1.testForward(conv1.output);
            thiRel1.forward(batch1.output);
            pool1.forward(thiRel1.output);
            conv2.forward(pool1.output);
            batch2.testForward(conv2.output);
            thiRel2.forward(batch2.output);
            span.forward(thiRel2.output);
            line.forward(span.output);
            rel.forward(line.output);
